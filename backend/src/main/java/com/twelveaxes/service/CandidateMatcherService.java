...
public class CandidateMatcherService {
    ...
    public List<CandidateMatch> findMatches(List<AxisResult> axes) {
        ...
        return data.getCandidates().stream().filter(Candidate::active)
                ...
                .limit(4).map(r -> { Candidate c=r.candidate(); return new CandidateMatch(c.id(),c.name(),c.role(),c.description(),c.party(),c.partyName(),c.ballotNumber(),c.runningMate(),c.imagePath(),c.imageSourceName(),c.imageSourceUrl(),c.imageNote(),r.score()); }).toList();
    }
